...
lower_yellow_thresh, upper_yellow_thresh = np.array([15,120,120]), np.array([25,255,255])
lower_white_thresh, upper_white_thresh = np.array([0,0,221]), np.array([180,255,255])
...
def canny_edge_detection(img):
    ...
    (thresh_canny_gray, output_canny_gray) = cv2.threshold(gray_img, 160, 255, cv2.THRESH_BINARY)
    output_canny_gray = cv2.GaussianBlur(output_canny_gray, (5, 5), 3)
    output_canny_gray = cv2.Canny(output_canny_gray, 180, 255)

    # make the edge wider so it falls inside the lanes
    kernel = np.ones((3, 3))
    dilate = cv2.dilate(output_canny_gray, kernel, iterations=1)

    binary_output_canny = np.zeros_like(output_canny_gray)
    binary_output_canny[(dilate == 255)] = 1

    return binary_output_canny


def hsv_thresh(img):
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

    lower_yellow = lower_yellow_thresh
    upper_yellow = upper_yellow_thresh

    yellow_lane = cv2.inRange(hsv, lower_yellow, upper_yellow)
    binary_output_yellow = np.zeros_like(yellow_lane)
    binary_output_yellow[(yellow_lane == 255)] = 1

    lower_white = lower_white_thresh
    upper_white = upper_white_thresh

    white_lane = cv2.inRange(hsv, lower_white, upper_white)
    binary_output_white = np.zeros_like(white_lane)
    binary_output_white[(white_lane == 255)] = 1

    return binary_output_yellow, binary_output_white
...
def find_edges(img, s_thresh=s_thresh, sx_thresh=sx_thresh, dir_thresh=dir_thresh, filters = 1):

    img = np.copy(img)

    # hsv and canny edge detection
    if filters == 1:
        # HSV filter
        binary_output_yellow, binary_output_white = hsv_thresh(img)

        # canny edge detection
        binary_output_canny = canny_edge_detection(img)

        # # gradient direction
        dir_binary = dir_threshold(img, sobel_kernel=3, thresh=dir_thresh)

        combined_binary = np.zeros_like(binary_output_canny)
        combined_binary[(((binary_output_yellow == 1) & (binary_output_canny == 1) & (dir_binary == 1)) | (
                    (binary_output_white == 1) & (binary_output_canny == 1) & (dir_binary == 1)))] = 1

        binary_final = combined_binary
    ...
    if filters == 2:
        ...
        binary_final = (combined_binary + c_bi)
...
def full_search(binary_warped, visualization=False):
    half_height = int(binary_warped.shape[0] / 2)
...
def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, binary_sub, threshold=0.6):
    ...
    frame_h = math.ceil(h*2.5)
    frame_w = math.ceil(w*2.34)
    whole_frame = np.zeros((frame_h,frame_w, 3), dtype=np.uint8)
...
def process_frame(img, visualization=False):
    ...
    img_binary = find_edges(img_undist, filters=1)
